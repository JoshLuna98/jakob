...
namespace Almacen_Tiempo_de_Ejecucion
{
    public partial class Almacen : Form
    {
        ...
        Panel Panel_Almacen = new Panel();
        public void Crear_Panel()
        {
            //Crea el panel de proveedores
            Panel_Almacen.Location = new System.Drawing.Point(0, 0);
            Panel_Almacen.Name = "puesto";
            Panel_Almacen.Size = new System.Drawing.Size(593, 309);
            Panel_Almacen.Visible = false;//Esta invisible hasta hacer clic en el boton de nuevo proveedor
            Panel_Almacen.BackColor = Color.Blue;
            Controls.Add(Panel_Almacen);
        }
        public Almacen()
        {
            InitializeComponent();
            this.BackColor = Color.Chocolate;
            Controls.Add(data1.dataalmacen());
            Controls.Add(label1.label1_Almacen());
            Controls.Add(label2.label2_Almacen());
            Crear_Botones();
            TextyMascaras();
            Crear_Panel();
            Componentes_Panel();
        ...
        public void Componentes_Panel()
        {
            ...
            Panel_Almacen.Controls.Add(boton_volver);
            Panel_Almacen.Controls.Add(boton7);
            Panel_Almacen.Controls.Add(label3.label1_Almacen_P());//Invocar el objeto de la forma y su funcion con las propiedades, 
            ////                                                     ////igual para los demas labels
            Panel_Almacen.Controls.Add(label4.label2_Almacen_P());
            Panel_Almacen.Controls.Add(label5.label3_Almacen_P());
            Panel_Almacen.Controls.Add(mascara3);
            Panel_Almacen.Controls.Add(mascara4);
            Panel_Almacen.Controls.Add(mascara5);
        ...
        public void evento_agregar(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(mascara3.Text) && !string.IsNullOrWhiteSpace(mascara4.Text) && !string.IsNullOrWhiteSpace(mascara5.Text))
            {
                try
                {
                    ...
                    comando = new SqlCommand("INSERT INTO ALMACENESTRELLAS (id_Cotizaciones, id_Trabajador,id_Inventario)  VALUES('" + this.mascara3.Text + " ','" + this.mascara4.Text + "','" + this.mascara5.Text + "')", conexion);
                    ...
                    cargar_datagrid(data1.dataalmacen());
                }
                catch (Exception p)
                {
        ...
        public void evento_volver(object sender, EventArgs e)
        {
            ...
            Panel_Almacen.Visible = false;

            data1.dataalmacen().Visible = true;
            label1.label1_Almacen().Visible = true;
            label2.label2_Almacen().Visible = true;
        ...
        public void Crear_Botones()
        {
            ...
            boton2.Click += new System.EventHandler(evento_nuevo_almacen);//Muestra el panel
        ...
        {
            int id = data1.dataalmacen().SelectedCells[0].RowIndex;
            actualiza_grid(id + 1);
            data1.dataalmacen().Refresh();
        }
        private void actualiza_grid(int id)
        {
            int renglon = data1.dataalmacen().SelectedCells[0].RowIndex;
            string tra = data1.dataalmacen().Rows[renglon].Cells[1].Value.ToString();
            string inv = data1.dataalmacen().Rows[renglon].Cells[2].Value.ToString();
            conexion.Open();
            string consulta = "update ALMACENESTRELLAS set id_Trabajador = '" + tra + "',id_Inventario = '" + inv + "' where id_Cotizaciones=" + id;
        ...
        {
            int pos;
            if (data1.dataalmacen().Rows.Count == 0)
            {
                MessageBox.Show("NO HAY ELEMENTOS");
            }
            else
            {
                pos = data1.dataalmacen().CurrentRow.Index;
                DialogResult dialogResult = MessageBox.Show("¿Desea eliminar El ID " + data1.dataalmacen()[0, pos].Value.ToString() + " ?", "Advertencia", MessageBoxButtons.YesNo);
                if (dialogResult == DialogResult.Yes)
                {
                    conexion.Open();
                    string cadena = "delete from ALMACENESTRELLAS where id_Cotizaciones=" + data1.dataalmacen()[0, pos].Value.ToString();
                    SqlCommand comando = new SqlCommand(cadena, conexion);
                    comando.ExecuteNonQuery();
                    MessageBox.Show("Se borró el id " + data1.dataalmacen()[0, pos].Value.ToString());
                    conexion.Close();
                    cargar_datagrid(data1.dataalmacen());
                }
        ...
        public void evento_buscar(object sender, EventArgs e)
        {
            ...
            try
            {
                if (mascara1.Text != string.Empty)
                {
                    ...
                    conexion.Open();
                    string consulta = "select * from ALMACENESTRELLAS where id_Cotizaciones like '%" + mascara1.Text + "%'";
                    comando = new SqlCommand(consulta, conexion);
                    SqlDataAdapter adaptador = new SqlDataAdapter(comando);
                    adaptador.Fill(tabla, "ALMACENESTRELLAS");
                    data1.dataalmacen().DataSource = tabla;
                    data1.dataalmacen().DataMember = "ALMACENESTRELLAS";
                    data1.dataalmacen().Refresh();
                    conexion.Close();
                }
                else if (mascara2.Text != string.Empty)
                {
                    ...
                    conexion.Open();
                    string consulta2 = "select * from ALMACENESTRELLAS where id_Trabajador like '%" + mascara2.Text + "%'";
                    comando = new SqlCommand(consulta2, conexion);
                    SqlDataAdapter adaptador2 = new SqlDataAdapter(comando);
                    adaptador2.Fill(tabla2, "ALMACENESTRELLAS");
                    data1.dataalmacen().DataSource = tabla2;
                    data1.dataalmacen().DataMember = "ALMACENESTRELLAS";
                    data1.dataalmacen().Refresh();
                    conexion.Close();
                }
        ...
        public void evento_nuevo_almacen(object sender, EventArgs e)
        {
            //Activa el panel y oculta los componentes de la forma
            Panel_Almacen.Visible = true;

            data1.dataalmacen().Visible = false;
            label1.label1_Almacen().Visible = false;
            label2.label2_Almacen().Visible = false;
        ...
        private void cargar_datagrid(DataGridView mostrar)
        {
            ...
            {
                conexion.Open();
                comando = new SqlCommand("select * from ALMACENESTRELLAS", conexion);
